...
출시 예정 게임을 소개하는 인터랙티브 랜딩페이지입니다. 작은 기계가 네 구역을 돌아다니고, 화면 밖의 눈·밸브·조명·전화·프린터·영상 플러그가 그 공간에 반응합니다. 게임 공개 정보는 탐험 완료 여부와 관계없이 접근할 수 있습니다.
...
- 바닥·물건·기록물을 클릭: 가구를 피해 해당 위치로 이동하거나 조사합니다.
- 방향키/WASD: 8방향 이동. E: 가까운 물건 조사. 화면의 방향 버튼도 지원합니다.
- 좌우 출입구: 다음/이전 구역으로 이동합니다.
- 외부 눈·밸브·조명: 방 안 기계, 조명과 음악의 음색·잔향을 바꿉니다.
- 전화·프린터·디스크: 짧은 이야기와 추가 메시지를 확인합니다.
- 영상 플러그: 화면과 탐험을 멈췄다가 같은 자리에서 연결합니다.
- 방송: 사용자 재생 후 시작하며 곡·음량·이전/다음 곡을 선택합니다. 외부 음원을 다운로드하지 않는 8개의 오리지널 합성 스케치입니다.
...
| `/records` | 기본 기록과 이번 방문에서 회수한 조각·기계 기록 |
...
각 맵은 36×7 공간입니다. `config.js`의 가구 크기는 충돌과 근접 판정의 기준입니다. 렌더 모델 크기를 바꿀 때 이 값도 맞춥니다. 출입구의 `visualOffset`과 `pickWidth`는 가장자리 안쪽에 그린 문과 클릭 영역을 일치시킵니다. 배경의 바닥 경계는 이미지 높이의 62%입니다. 캐릭터는 같은 관절 좌표를 8방향으로 투영하며, 실제 이동 거리로 보행을 진행합니다. 발을 딛는 구간은 몸의 이동을 상쇄하고, 되돌리는 구간은 발을 들어 올립니다. 캔버스 내부 비율은 표시 영역과 일치시켜 캐릭터가 눌리지 않도록 합니다.
